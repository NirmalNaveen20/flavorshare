...
export const createComment = (data) => async (dispatch) => {
  ...
  const res = await fetch(
    `http://localhost:5454/api/comments/create/${data.postId}`,
    {
      method: "POST",
...
export const findPostComment = (data) => async (dispatch) => {
  const res = await fetch(`http://localhost:5454/api/comments/${data.postId}`, {
    ...
    const resData = await res.json();
    console.log("find post comment ", resData);
    dispatch({ type: GET_POST_COMMENT, paylod: resData });
  
};

export const likeComment = (data) => async (dispatch) => {
  const res = await fetch(
    `http://localhost:5454/api/comments/like/${data.commentId}`,
    {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
...
export const unLikeComment = (data) => async (dispatch) => {
  const res = await fetch(
    `http://localhost:5454/api/comments/unlike/${data.commentId}`,
    {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
